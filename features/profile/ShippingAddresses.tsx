    ...
    <div className="min-h-dvh w-full overflow-x-hidden bg-[linear-gradient(180deg,#e0f2f1_0%,#f7f9fb_52%,#eef3f4_100%)] pb-[calc(7rem+env(safe-area-inset-bottom))] text-[#191c1e]">
      ...
      <main className="mx-auto w-full max-w-mobile px-6 pt-8">
        <section className="rounded-[28px] border border-white/40 bg-white/70 p-6 shadow-[0_12px_40px_rgba(0,96,103,0.08)] backdrop-blur-[24px]">
          ...
          <Link href={getShippingAddressHref("new=1", returnTo)} className="flex h-12 w-full items-center justify-center gap-2 rounded-full border border-primary/20 bg-white/70 text-sm font-bold text-primary shadow-sm active:scale-[0.99]"><Plus aria-hidden="true" className="size-4" />เพิ่มที่อยู่ใหม่</Link>
          {showForm ? <ShippingAddressForm address={editingAddress} /> : null}
        </section>
        <section className="mt-8 space-y-4">
          {addresses.length === 0 ? <EmptyState title="ยังไม่มีที่อยู่จัดส่ง" body="เพิ่มที่อยู่ก่อนสร้างคำสั่งซื้อ" icon={<MapPin aria-hidden="true" className="size-5" />} /> : addresses.map((address) => <AddressCard key={address.id} address={address} returnTo={returnTo} />)}
        </section>
...
function Header({ returnTo }: { returnTo?: string }) {
  return <header className="sticky top-0 z-header bg-white/70 shadow-[0_0_40px_rgba(0,123,131,0.06)] backdrop-blur-[24px]"><div className="mx-auto flex h-[82px] w-full max-w-mobile items-center gap-4 px-7"><Link href={(returnTo ?? "/profile") as Route} aria-label={returnTo ? "กลับไปข้อมูลบัญชี" : "กลับไปหน้าโปรไฟล์"} className="flex size-10 items-center justify-center rounded-full text-primary"><ArrowLeft aria-hidden="true" className="size-6" strokeWidth={2.4} /></Link><div className="min-w-0"><p className="text-[22px] font-bold tracking-wide text-primary">ที่อยู่จัดส่ง</p><p className="truncate text-xs font-medium uppercase tracking-[0.14em] text-[#3e494a]/60">ข้อมูลการจัดส่ง</p></div></div></header>;
}
