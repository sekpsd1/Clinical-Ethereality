...
export function ShippingAddresses({ addresses, editingAddress, showForm, returnTo }: { addresses: ShippingAddressView[]; editingAddress?: ShippingAddressView; showForm: boolean; returnTo?: string }) {
  ...
  return (
    <div className="min-h-dvh w-full overflow-x-hidden bg-[linear-gradient(180deg,#e0f2f1_0%,#f7f9fb_52%,#eef3f4_100%)] pb-[calc(7rem+env(safe-area-inset-bottom))] text-[#191c1e]">
      <Header returnContext={returnContext} />
      <main className="mx-auto w-full max-w-mobile px-6 pt-8">
        <section className="rounded-[28px] border border-white/40 bg-white/70 p-6 shadow-[0_12px_40px_rgba(0,96,103,0.08)] backdrop-blur-[24px]">
          ...
          <Link href={getShippingAddressHref("new=1", returnContext?.href)} className="flex h-12 w-full items-center justify-center gap-2 rounded-full border border-primary/20 bg-white/70 text-sm font-bold text-primary shadow-sm active:scale-[0.99]"><Plus aria-hidden="true" className="size-4" />เพิ่มที่อยู่ใหม่</Link>
          {showForm ? <ShippingAddressForm address={editingAddress} /> : null}
        </section>
        <section className="mt-8 space-y-4">
          {addresses.length === 0 ? <EmptyState title="ยังไม่มีที่อยู่จัดส่ง" body="เพิ่มที่อยู่ก่อนสร้างคำสั่งซื้อ" icon={<MapPin aria-hidden="true" className="size-5" />} /> : addresses.map((address) => <AddressCard key={address.id} address={address} returnTo={returnContext?.href} />)}
        </section>
        {returnContext ? <Link href={returnContext.href as Route} className="mt-6 flex h-12 items-center justify-center rounded-full bg-primary text-sm font-bold text-white">{returnContext.ctaLabel}</Link> : null}
      </main>
    </div>
  );
}

function Header({ returnContext }: { returnContext?: ReturnContext }) {
  return <header className="sticky top-0 z-header bg-white/70 shadow-[0_0_40px_rgba(0,123,131,0.06)] backdrop-blur-[24px]"><div className="mx-auto flex h-[82px] w-full max-w-mobile items-center gap-4 px-7"><Link href={(returnContext?.href ?? "/profile") as Route} aria-label={returnContext?.headerLabel ?? "กลับไปหน้าโปรไฟล์"} className="flex size-10 items-center justify-center rounded-full text-primary"><ArrowLeft aria-hidden="true" className="size-6" strokeWidth={2.4} /></Link><div className="min-w-0"><p className="text-[22px] font-bold tracking-wide text-primary">ที่อยู่จัดส่ง</p><p className="truncate text-xs font-medium uppercase tracking-[0.14em] text-[#3e494a]/60">ข้อมูลการจัดส่ง</p></div></div></header>;
}

function AddressCard({ address, returnTo }: { address: ShippingAddressView; returnTo?: string }) {
  return (
    <article className="rounded-[24px] border border-white/50 bg-white/75 p-5 shadow-[0_10px_30px_rgba(0,96,103,0.06)] backdrop-blur-[24px]">
      ...
      <div className="mt-4 flex gap-3"><Link href={getShippingAddressHref(`edit=${encodeURIComponent(address.id)}`, returnTo)} className="flex h-10 flex-1 items-center justify-center rounded-full border border-primary/20 text-xs font-bold text-primary">แก้ไข</Link>{!address.isDefault ? <form action={setDefaultShippingAddressAction} className="flex-1"><input type="hidden" name="addressId" value={address.id} /><button type="submit" className="h-10 w-full rounded-full bg-primary/10 text-xs font-bold text-primary">ตั้งเป็นค่าเริ่มต้น</button></form> : null}</div>
    </article>
  );
}

function getShippingAddressHref(query: string, returnTo?: string): Route {
  const returnToQuery = returnTo ? `&returnTo=${encodeURIComponent(returnTo)}` : "";
  return `/profile/shipping-addresses?${query}${returnToQuery}` as Route;
}
...
function getReturnContext(returnTo?: string): ReturnContext | undefined {
  if (returnTo === "/profile/settings?section=account") {
    return {
      href: returnTo,
      headerLabel: "กลับไปข้อมูลบัญชี",
      ctaLabel: "กลับไปข้อมูลบัญชี"
    };
  }

  if (isOrderAddressSelectionReturn(returnTo)) {
    return {
      href: returnTo,
      headerLabel: "กลับไปเลือกที่อยู่ในคำสั่งซื้อ",
      ctaLabel: "กลับไปเลือกที่อยู่ในคำสั่งซื้อ"
    };
  }
}

function isOrderAddressSelectionReturn(returnTo?: string): returnTo is string {
  return returnTo === "/store/checkout" || /^\/store\/(?:[^/?]+|prescriptions\/[^/?]+)$/.test(returnTo ?? "");
}
