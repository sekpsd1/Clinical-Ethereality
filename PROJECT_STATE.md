...
- Reusable mobile layout primitives now include `Screen`, `SafeArea`, `Button`, `IconButton`, `TextField`, `SearchField`, `EmptyState`, `PaymentStatusBadge`, `OrderTrackingTimeline`, `NotificationItem`, and `ProfileSettingsItem`; continue extracting unchecked Stitch primitives without redesigning finalized screens.
