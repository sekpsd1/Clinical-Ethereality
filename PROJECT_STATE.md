...
- Admin shell: `/admin` uses a dedicated responsive operational layout with persistent sidebar navigation and a logout action on desktop, the existing compact bottom navigation on mobile, and data-backed queue modules for role approvals, consultations, payments, prescriptions, orders, stock, moderation, and audit activity.
...
- Intake templates: `DOCTOR_INTAKE_TEMPLATE.md`, `CONSULT_ASSESSMENT_INTAKE_TEMPLATE.md`, `PRODUCT_CATALOG_TEMPLATE.csv`, and `PHARMACIST_INTAKE_TEMPLATE.md` now provide owner-sendable templates for collecting the remaining doctor, pre-doctor assessment, product catalog, and pharmacy/pharmacist data without committing sensitive exact values.
