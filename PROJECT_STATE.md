...
- Persisted customer shipping addresses and an immutable order shipping-address snapshot; the current placeholder address was removed from checkout, but orders do not yet capture a fulfillable destination.
...
- Community images can temporarily use private Plesk host storage through `COMMUNITY_UPLOAD_DIR`; migrate file bytes to Cloudinary/S3-compatible object storage before host capacity or multi-instance deployment makes local disk unsuitable. Final Community rules/moderation policy still requires owner approval. Persisted shipping addresses remain incomplete. Doctor production work now mainly remains Zoom credential/UAT validation and deciding whether polling should be replaced by a dedicated realtime transport.
