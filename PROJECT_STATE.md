...
- Reusable mobile layout primitives now include `Screen`, `SafeArea`, `Button`, `IconButton`, `TextField`, `SearchField`, `EmptyState`, `PaymentStatusBadge`, `OrderTrackingTimeline`, `NotificationItem`, `ProfileSettingsItem`, `ArticleCard`, and `CommunityPostCard`; continue extracting unchecked Stitch primitives without redesigning finalized screens.
