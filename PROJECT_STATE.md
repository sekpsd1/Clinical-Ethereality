...
- Doctor consultation queue cards now use the immutable `slotMinutes` snapshot written in the existing `consultation.book_slot` audit event inside the booking transaction, so duration remains available after expired/cancelled consultations release and delete their slot lock. This is an interim no-migration solution: legacy audit entries may recover from `availabilityId` only when the related `DoctorAvailability.updatedAt` is not newer than the booking audit; otherwise they show `ยังไม่ระบุ`. Behavioral coverage confirms booking Audit records `slotMinutes`, duration remains readable after slot-lock deletion, and Production legacy recovery returns the unchanged availability duration. After a Production Prisma baseline is approved, move this snapshot to `Consultation` as the normalized long-term source. Local verification passed 356 unit tests (including snapshot, legacy, malformed-metadata, deterministic multi-audit, and all-status card coverage), a focused Pixel 5 Doctor queue E2E smoke test, lint, typecheck, and a 61-route production build.
