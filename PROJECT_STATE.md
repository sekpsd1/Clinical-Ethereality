...
- Doctor consultation queue cards now show the actual appointment duration from the existing `DoctorAvailability.slotMinutes` record linked through the consultation slot lock's `availabilityId`; all statuses use the same card field and incomplete historic links show `ยังไม่ระบุ` rather than an invented duration. This uses the existing query/model only, with no schema or migration change. Local verification passed 349 unit tests (including duration/card coverage), a focused Pixel 5 Doctor queue E2E smoke test, lint, typecheck, and a 61-route production build.
