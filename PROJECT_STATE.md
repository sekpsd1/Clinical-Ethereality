...
- Customer profile/settings polish: `/profile`, `/profile/settings`, and `/profile/rewards` now use clearer Thai customer copy for membership, profile stats, consent status badges, reward balance, reward rules, and ledger expiry labels while preserving existing profile support-screen layouts.
...
- First hosted Plesk smoke deployment is pending the new Plesk host. The temporary cPanel host proved the standalone app and LINE Login build path but is not the preferred ongoing deployment target because of shared-host resource limits.
...
Phase 12 quality work is complete, and the staff invitation, consent acceptance, pharmacy SOP draft, client SOP review message, production PromptPay intake, pre-doctor assessment intake, and remaining client intake template foundations are now in place. Permission unit tests, feature validation schema tests, reusable Stitch UI primitive tests including `Screen`/`SafeArea`, form/button/empty-state primitives, order payment/tracking primitives, notification/profile row primitives, community card primitives, the comment composer primitive, and the bottom sheet primitive, order fulfillment service tests, doctor-issued prescription service tests, consultation payment verification service tests, manual payment review service tests, provider slip verification service tests, protected workflow integration tests, footer/navigation checks, mobile LINE LIFF viewport smoke coverage, Playwright smoke tests, Vercel preview deployment configuration, Plesk Node.js standalone deployment readiness, Plesk dry-run checklist, artifact script, staging environment configuration, production environment launch gates, backup/restore readiness plans, Sentry error monitoring wiring, admin compliance readiness review, versioned PDPA/Terms/health consent acceptance, the draft pharmacy/prescription fulfillment SOP, owner-facing client review message, PromptPay identifier receipt, all four Stitch-exported assessment pages, and the persisted assessment data flow are in place. The 2026-06-08 client intake bundle now provides one real doctor profile/photo, physician-license proof image, product catalog details/images, clinic/facility license direction, and payment reference data. Client has confirmed that prescription-required products can be purchased by attaching a prescription without an additional document-review step after attachment. The received doctor and product catalog data are now mapped into local seed/assets without committing exact sensitive license values, assessment recommendations now hand off to the received real doctor profile while preserving manual doctor choice, in-app doctor-issued prescriptions can now be used for prescription-required ordering without a pharmacist verification gate, external prescription attachment metadata can now link owner-managed storage URLs to orders without storing file bytes in the database, doctor-issued prescription rules, consultation payment verification transitions, manual payment review rules, and provider slip verification persistence now live in tested domain services, persisted in-app consultation chat is now implemented separately from LINE chat on the existing Prisma/MySQL foundation, admin integration readiness now shows owner-managed credential gaps without exposing secret values, customer booking now has database-backed slot locks that mark booked slots as unavailable, expired pending-payment slot holds now release automatically, consultation payment status polish now covers pending/verified/rejected/expired/closed states, doctor consultation workflow polish now clarifies readiness/payment/assessment/chat/prescription actions, doctor patient-log polish now makes patient history Thai and masks raw LINE IDs in the patient-log list, admin payment review now shows clearer evidence/provider metadata in Thai, staff/admin copy cleanup now covers admin notifications, moderation, products, compliance readiness, integration readiness, DB-offline states, and admin navigation, pharmacist order language/status polish now makes the fulfillment queue Thai and clearer without adding a document-review gate, customer profile/settings/rewards polish now removes remaining English-facing profile copy, shared `InfoTile` now reduces duplicated operational data-row UI across admin, doctor, and pharmacist screens, and admin product image fields now clarify already-uploaded image links while warning against sensitive document links. The temporary cPanel deployment proved the standalone app health check and LINE Login redirect but exposed shared-host resource limits, so the client is moving to a new Plesk host and `PLESK_MIGRATION_HANDOFF.md` is prepared for that move. FDA numbers, EasySlip credentials, realtime chat provider selection, Zoom/file-storage credentials, and final Plesk production credentials remain open. Next recommended step: wait for the new Plesk host, then complete the first hosted Plesk smoke deployment using the migration handoff; meanwhile continue only local-only polish that does not require production hosting or new architecture.
