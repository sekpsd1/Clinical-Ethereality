...
- Staff invite flow: `/staff-invite/doctor`, `/staff-invite/pharmacist`, and `/staff-invite/admin` create pending review requests from authenticated LINE users, preserve admin approval as the final gate, and write audit metadata for invite requests. Staff review status is held on the doctor/pharmacist profile so the applicant's customer session remains usable until approval; successful submissions render a Thai pending-review state.
