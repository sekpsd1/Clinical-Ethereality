...
- Provider slip verification service extraction: `features/payments/service.ts` now owns provider result mapping, payment/order persistence, customer notification, hosted slip attachment metadata, duplicate attachment checks, and audit logging after SlipOK/EasySlip verification; `/api/payments/verify-slip` stays focused on auth, request validation, provider calls, and response shaping.
...
- The current `clinical_ethereality_host_copy` snapshot has no product or order rows and predates the structured-product columns from `20260727160000_add_structured_product_details`; Customer Store QA therefore covered reviewed fallback/empty states, but a fully data-backed cart-to-order/payment run still requires a refreshed Plesk copy or applying the current schema plus a client-approved test catalog.
