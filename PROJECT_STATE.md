...
Customer Store safety completion and integration validation.
...
- Clinic-as-pharmacy decision update: client says the clinic is used as the dispensing storefront and doctors prescribe directly; if a medicine requires a prescription, the customer can attach the prescription and purchase in the system without an additional document-review step after attachment. The in-app doctor-issued prescription path now lets customers order prescription-required products without a pharmacist verification gate while preserving prescription/order/payment/audit linkage. External prescription attachment metadata and local file-selection UX stub are implemented for owner-managed storage URLs; production file bytes still require Cloudinary or S3-compatible storage setup.
