...
- Clinic-as-pharmacy decision update: client says the clinic is used as the dispensing storefront and doctors prescribe directly; if a medicine requires a prescription, the customer can attach the prescription and purchase in the system without an additional document-review step after attachment. This is documented as an approved intake direction; existing code still has pharmacist verification paths, so a future scoped implementation should adjust prescription-required purchasing while preserving audit, ownership, and secure file-storage controls.
