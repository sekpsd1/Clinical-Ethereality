...
- Formal booking slot locking: consultation booking now creates a `ConsultationSlotLock` row with a unique `doctorId + scheduledAt` constraint before creating the pending-payment consultation, and the booking screen marks locked or already active consultation slots as unavailable so customers cannot select booked times
...
- Broader booking integration now has database-backed slot locking for customer booking; future refinements can release locks on explicit cancellation/expiry and add richer admin schedule management.
