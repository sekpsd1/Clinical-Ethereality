...
import { Bell, ClipboardCheck, PackageCheck } from "lucide-react";
...
export function PharmacistShell({ children }: { children: React.ReactNode }) {
      ...
      <header className="sticky top-0 z-header border-b border-border/70 bg-white/85 px-4 pb-3 pt-[calc(0.75rem+env(safe-area-inset-top))] backdrop-blur-topbar">
        <div className="mx-auto flex w-full max-w-mobile items-center justify-between gap-3">
          <div className="min-w-0">
            <p className="text-label font-bold uppercase text-primary">เภสัชกร</p>
            <h1 className="truncate font-headline text-xl font-bold text-text">ตรวจใบสั่งยาและจัดยา</h1>
          </div>
          <button
            type="button"
