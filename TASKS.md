...
- [x] Complete EasySlip v2 Production pre-credential readiness locally: align the provider-neutral adapter with the documented `rawSlip` response plus `matchAccount`, `matchAmount`, and `checkDuplicate`; preserve Store/Consultation fail-closed verification, normalized unique references, and no-sensitive-log rules; document owner/Plesk/rollback steps. Private browser-to-server slip upload is now an independent Plesk MVP; provider/API key/sandbox UAT remains blocked pending provider confirmation.
...
- [ ] Configure EasySlip/Payment Verification credentials and complete a non-monetary provider UAT. Private Plesk payment-slip upload is a separate completed MVP; do not use real money or slips before the provider sandbox/fixture is approved.
...
- [x] Add refund stock-release and real refund handling after Payment Integration/EasySlip is ready; unpaid customer cancellation remains separate from a refund
