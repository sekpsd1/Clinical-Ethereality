...
- [ ] Review the code-only fail-closed SMS OTP partial-schema reconciliation. Production execution requires a fresh verified database backup, maintenance window, exact one-time application-runtime target, one reconciliation restart that intentionally leaves the app stopped, target removal, one normal restart, health/schema verification, and restore-on-failure. Do not run the original migration blindly or use Plesk Run Node.js commands for this path.
