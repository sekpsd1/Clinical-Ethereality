...
- [x] Add Shared Plesk Prisma migration runner scripts: production status/deploy must use `npm run db:migrate:status` and `npm run db:migrate:deploy` through the Node.js command runner, with `DATABASE_URL` supplied only by the application environment; do not use Scheduled Tasks, temporary `.env` files, direct secret-bearing commands, or `prisma db push`
