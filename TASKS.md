...
- [x] Add read-only completed-Consultation chat history and a server-authorized UTF-8 Thai `.txt` download for the owning active Customer and active approved assigned Doctor, with deterministic ordering, minimized view/download audits, generic export metadata, explicit exclusion of Zoom Chat, and no changes to live sending or five-year retention
- [x] Add fail-closed provider readiness checks and accessible Thai retry states to protected Doctor/Admin recording actions so temporary Zoom failures do not navigate to raw JSON, without changing recording authorization, proxy/range semantics, retention, storage, or provider settings
...
- [x] Add and deploy the collapsible Consultation recording panel to assigned-Doctor queue cards and Admin customer-detail appointment history, with Thai file labels, time/duration/size/retention metadata, protected view/download actions, explicit Zoom-processing empty state, and authenticated Production visual smoke on both surfaces
- [x] Implement and deploy the mobile recording external handoff at application commit `d23337e`: fresh same-origin view/download tickets, fragment-only public bridge, Android Chrome/iOS external launch, exact mode/path cookies, per-request Admin/assigned-Doctor reauthorization, MP4/M4A byte-range proxying, and one logical access audit per external session. Plesk non-migration preflight, dependency and Zoom-client install/build, the 65-page host build, one restart, health HTTP 200, and bounded no-data denial smoke passed; Android/iOS Doctor/Admin device UAT remains separately gated
...
- [x] Implement the replacement one-time Doctor invitation lifecycle: active-Admin issue/list/revoke, 24-hour hash-only fragment link, LINE Customer claim, pending-review routing and workflow denial, existing Admin-only professional review, and additive migration; Production release and real LINE UAT remain separately gated
- [x] Define customer, doctor, pharmacist, and admin roles
...
- [x] Implement the code-only verified Zoom attendance completion gate with hashed role-scoped `customerKey` credentials, replay-safe participant join/left evidence, two-party completion, controlled 10-minute no-show handling, Doctor/Customer status UI, audit/notification coverage, and an additive migration; Production migration/deploy and Zoom scope/event activation remain separately gated
- [x] Replace the fixed 10-minute completion/no-show threshold with the immutable booked duration: new 15-minute and stored 30/45/60-minute appointments require one continuous verified Doctor Zoom interval counted from the later of scheduled time or join time, while missing/invalid legacy duration falls back to 30 minutes. Normal completion additionally requires prior same-meeting Doctor/Customer overlap; disconnect/rejoin intervals are never summed. The existing row lock, Serializable transaction, identity gate, 5-second queue refresh, prescriptions, recordings, payments, and Zoom handoff remain unchanged. This is code-only with no schema, migration, Production mutation, push, or deployment.
- [x] Supersede the full-duration rule for normal completion only: after verified Doctor/Customer overlap in the same meeting and matching leave evidence for every tracked participant session, the assigned Doctor may complete immediately even before 15 minutes. Keep live completion visible/disabled until both sides have left; keep incomplete attendance fail-closed and preserve the full booked-duration rule for no-show. Code-only; Production release and authenticated UAT remain separately gated.
- [ ] After explicit coordinated-release approval: take a fresh Production backup, stage the matching source/build, use the guarded one-time target to apply `20260906120000_add_zoom_attendance_gate` before that runtime starts, verify/remove the target, then add `meeting:read:participant:admin` and subscribe the existing Zoom endpoint to `meeting.participant_joined` / `meeting.participant_left`; run a new Websthai booking UAT and do not reuse Honey or any completed appointment
...
- [x] Fix Customer notification/advice-log isolation: enforce Customer role on page/query/actions, suppress staff-addressed historical notifications, bind consultation destinations to ownership-scoped opaque IDs, replace static Doctor/advice/medicine/PDF content with completed Consultation data, and return the close action to the owned appointment detail. Production application commit `72c1488` passed Plesk Pull/Deploy, non-migration preflight, dependency install, Zoom client build, host build, one restart, HTTP 200 health, and anonymous route smoke; authenticated Customer regression UAT remains separately gated
...
- [x] Replace the LINE Mini App iframe launch with a safe external-browser Zoom handoff: LIFF `external: true`, a hashed 2-minute one-time ticket, a role/Consultation-bound HttpOnly API cookie, repeated active-account/appointment checks, and mandatory camera/microphone preflight before join credentials or attendance `customerKey` are issued. Attendance webhook/completion logic is unchanged; deployment and Doctor/Customer device UAT remain separately gated.
...
- [x] Deploy the Doctor `ยืนยันจบการปรึกษา` visible-disabled state at application commit `da528fa`: non-migration preflight, 65-route Plesk host build, one restart, health HTTP 200, route smoke, and authenticated read-only Doctor verification passed without a Consultation or data mutation; exact 390×844 Production-browser verification and a fresh two-party Doctor/Customer UAT remain pending.
...
- [x] Reconcile Store inventory exactly once: release pending reservations, restore consumed units from paid/fulfillment states, and make inconsistent order/payment/inventory state fail closed.
...
## New consultation duration default (deployed, 2026-09-14)
