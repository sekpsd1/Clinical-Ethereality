...
- [ ] Activate the host-authoritative five-minute reservation cleanup schedule. Plesk customer-level Scheduled Tasks is chrooted: safe Production preflight confirmed the application root but both `node` and `/opt/plesk/node/24/bin/node` are unavailable there, and the Node application environment is not inherited. No invalid task was saved and no cleanup call was made. The hosting operator must create a non-chrooted system-user task or expose the approved Node runtime plus protected environment boundary without placing the secret in the command or logs; then verify one natural scheduled run and the 15-minute readiness check.
