...
- [x] Implement the local-only mobile recording external handoff: fresh same-origin view/download tickets, fragment-only public bridge, Android Chrome/iOS external launch, exact mode/path cookies, per-request Admin/assigned-Doctor reauthorization, MP4/M4A byte-range proxying, and one logical access audit per external session; release and device UAT remain separately gated
