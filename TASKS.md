...
- [ ] Review the minimal code-only collation-safe reconciliation fix: select static DDL only for allowlisted `User.id` `utf8mb4` collations, verify FK-column parity, and never alter `User` or the database default; Production remains unchanged pending approval.
