...
- [ ] Review the minimal code-only `user_table` sub-reason follow-up: add only closed-enum `reasonDetail` metadata from the existing inspection, with no new query or reconciliation behavior change; Production remains unchanged pending approval.
