...
- [ ] Configure ThaiBulkSMS owner-managed Production keys: `SMS_OTP_PROVIDER`, `SMS_OTP_API_KEY`, `SMS_OTP_API_SECRET`, and `SMS_OTP_CHALLENGE_ENCRYPTION_KEY`. Do not record values in source, logs, or documentation.
- [ ] Run one Controlled Production SMS OTP UAT after configuration: verify progressive booking-only data collection, one real OTP send, successful verification, server-side booking gate, and health; do not create a Consultation, Payment, or Zoom session in this UAT.
