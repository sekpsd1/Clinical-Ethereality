...
- [ ] Review the code-only reconciliation observability follow-up: persist only allowlisted stage/status/action metadata atomically in a private non-public status artifact, require diagnostics readiness before database access, preserve the mutation-started boundary after DDL, keep Passenger/raw database/Prisma output out of the operator path, and verify the private path through the no-environment/no-database probe with both target keys absent. Merge/deploy, the probe, and any new target attempt require separate approvals.
