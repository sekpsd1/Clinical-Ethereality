...
- [ ] Review the minimal code-only precondition component follow-up after Production safely rejected before DDL: add only closed-enum `reasonComponent` metadata to the private artifact so migration state, User table/columns/indexes, challenge absence, or inspection can be distinguished without raw schema/value/error output; DDL/migration behavior and Production remain unchanged.
