...
- [x] Add an authenticated scheduled/global reservation cleanup worker so abandoned orders are reclaimed without waiting for the same customer to return; deployment still requires an owner-managed `STORE_RESERVATION_CLEANUP_SECRET`, Plesk Scheduled Task configuration, and Production UAT
