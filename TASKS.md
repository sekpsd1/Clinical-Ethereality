...
- [ ] After separate approval, add the persistent OTP challenge/rate-limit model, `phoneVerifiedAt`, normalized-phone uniqueness decision, protected request/verify routes, audit records, and booking/pre-Zoom gates. This requires an additive migration and controlled Staging UAT; no Production SMS is authorized.
...
- [x] Add an authenticated scheduled/global reservation cleanup worker so abandoned orders are reclaimed without waiting for the same customer to return. Production RCA on 2026-08-13 proved GitHub-hosted schedule delivery was delayed by 34-68 minutes despite the five-minute cron expression, so Plesk Scheduled Tasks is now the authoritative five-minute scheduler and GitHub Actions is retained only as a manually dispatched recovery path. The host runner uses the existing authenticated endpoint, a two-minute stale lock, idempotent database cleanup, a private last-success status file, a 15-minute readiness threshold, safe allowlisted output, and no secret in its command or logs.
...
- [ ] Complete a separately controlled Consultation Real-slip UAT (including PromptPay, provider verification, and webhook) before Full Doctor Flow Production UAT; SlipOK key-name readiness and `log=true` privacy approval are confirmed, while EasySlip remains inactive fallback
