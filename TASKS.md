...
- [x] Keep the Doctor `ยืนยันจบการปรึกษา` action visible but natively disabled and non-submitting while a live Consultation lacks eligible Zoom attendance, retain the status/reason in the same control card, and preserve normal/no-show/scheduled/completed behavior and queue ordering before the prescription section; code and tests are complete, while review/release remain separate.
