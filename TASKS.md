...
- [ ] Implement the approved progressive customer verification flow locally: retain low-friction LINE Login; require legal/full name, date of birth, and a Thai mobile number only when confirming a doctor booking; request and verify SMS OTP; then reuse the verified profile for later bookings and consultations. A phone change must invalidate verification. No ID card, passport, or face/selfie upload or Admin document review is permitted. The local implementation includes an additive migration only; applying it, configuring `SMS_OTP_PROVIDER`, `SMS_OTP_API_KEY`, and `SMS_OTP_API_SECRET`, sending SMS, and Controlled Staging/Production UAT require separate approvals.
