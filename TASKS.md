...
- [ ] Deploy the reviewed SMS OTP adapter fix under a separate approval, then request separate approval for Controlled Production OTP UAT attempt #2. Verify one provider request, successful verification, server-side booking gate, and health; do not create a Consultation, Payment, or Zoom session.
