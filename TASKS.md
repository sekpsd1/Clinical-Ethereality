...
- [x] Add a separate Test-only Zoom fixture runner with exact test-app, deployment-marker, database/user hash, LINE-backed account hash, migration/schema, and unrelated-integration fail-closed gates; idempotent Serializable create/cleanup; safe closed-enum output; and a provisioning/mobile-UAT runbook. Test deployment and execution remain separately gated.
- [x] Add a mandatory read-only Test database pre-migration guard that runs on a blank database, queries only `DATABASE()` / `CURRENT_USER()`, verifies the Test-only identity by timing-safe digest, rejects every migration/reconciliation target, and emits allowlisted output before `db:migrate:deploy`.
- [x] Add a separate Test-only LINE-account bootstrap with hashed account selection, precheck/apply/verify fingerprint flow, strict empty-state and business-record boundaries, Serializable synthetic Customer/Doctor setup, exact audit provenance, and replay-safe verification; never seed or promote these identities to Production.
