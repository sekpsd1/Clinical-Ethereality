...
- [x] Connect Store private-slip upload to immediate provider verification locally: after the authenticated owner upload is persisted and CAS-claimed, the server rereads and validates only the private bytes, submits the Payment amount to the selected provider once, and applies verified/rejected outcomes through the existing audited payment/order/inventory service. Storage/provider/persistence failure remains `pending_review` for Admin fallback; no raw response, secret, path, QR/account data, or file bytes are returned or logged. Mock-only tests passed without a real provider request, slip, transfer, migration, deployment, or Production mutation.
